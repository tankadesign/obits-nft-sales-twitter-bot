...
export class Erc721SalesService extends BaseService {
  private readonly logger = new Logger(Erc721SalesService.name);
    ...
    this.provider.on(
      ...
      (event) => {
        this.getTransactionDetails(event).then(async (res) => {
          ...
          if (res?.ether || res?.alternateValue) {
            await this.tweet(res);
          } else if (config.includeFreeMint) {
            // If free mint is enabled we can tweet 0 value
            await this.tweet(res);
          } else {
            this.logger.log('***** WE DON’T TWEET FREE TRANSFERS! *****');
          }
        });
      },
    );
  ...
  async getTransactionDetails(tx: ethers.Event): Promise<any> {
    ...
    try {
      ...
      if (code !== '0x') {
        this.logger.log(
          `contract detected for ${tx.transactionHash} event index ${tx.logIndex}`,
        );
        return;
      }
      ...
      this.logger.log(`handling ${transactionHash}`);
      ...
      const LR = receipt.logs
        .map((log: any) => {
          if (
            log.address.toLowerCase() === looksRareContractAddress.toLowerCase()
          ) {
        ...
        .filter(
          (log: any) =>
            (log?.name === 'TakerAsk' || log?.name === 'TakerBid') &&
            log?.args.tokenId == tokenId,
        );
      this.logger.log(`-> LR receipt parsed: ${LR.length}`);

      const NFTX = receipt.logs
        .map((log: any) => {
          // direct buy from vault
          if (
            log.topics[0].toLowerCase() ===
            '0x1cdb5ee3c47e1a706ac452b89698e5e3f2ff4f835ca72dde8936d0f4fcf37d81'
          ) {
            const relevantData = log.data.substring(2);
            const relevantDataSlice = relevantData.match(/.{1,64}/g);
            return (
              BigInt(`0x${relevantDataSlice[1]}`) / BigInt('1000000000000000')
            );
          } else if (
            log.topics[0].toLowerCase() ===
            ...
            if (
              !parsedLog.args.nftIds.filter(
                (n) => BigInt(n).toString() === tokenId,
              ).length
            ) {
              return;
            }
            ...
            const buys = receipt.logs
              .filter(
                (log2: any) =>
                  log2.topics[0].toLowerCase() ===
                  '0xf7735c8cb2a65788ca663fc8415b7c6a66cd6847d58346d8334e8d52a599d3df',
              )
              .map((b) => {
      ...
      this.logger.log(`-> NFTX receipt parsed: ${NFTX.length}`);

      // Check all marketplaces specific events to find an alternate price
      // in case of sweep, multiple buy, or bid

      const NLL = receipt.logs
        .map((log: any) => {
          if (
            log.topics[0].toLowerCase() ===
            '0x975c7be5322a86cddffed1e3e0e55471a764ac2764d25176ceb8e17feef9392c'
          ) {
            const relevantData = log.data.substring(2);
      ...
      this.logger.log(`-> NLL receipt parsed: ${NLL.length}`);

      const X2Y2 = receipt.logs
        .map((log: any, index: number) => {
          if (
            ...
            let amount =
              BigInt(`0x${dataSlices[12]}`) / BigInt('1000000000000000');
            if (amount === BigInt(0)) {
              amount =
                BigInt(`0x${dataSlices[26]}`) / BigInt('1000000000000000');
            }
            return amount;
          }
        })
      ...
      this.logger.log(`-> X2Y2 receipt parsed: ${X2Y2.length}`);

      const BLUR_IO = receipt.logs
        .map((log: any) => {
        ...
        .filter(
          (l) =>
            l?.name === 'OrdersMatched' &&
            l?.args.buy.tokenId.toString() === tokenId,
        );

      const OPENSEA_SEAPORT = receipt.logs
        .map((log: any) => {
          if (
            log.topics[0].toLowerCase() ===
            ...
            let amounts = logDescription.args.consideration.map((c) =>
              BigInt(c.amount),
            );
            // add weth
            const wethOffers = matchingOffers.map((o) =>
              o.token === '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2' &&
              o.amount > 0
                ? BigInt(o.amount)
                : BigInt(0),
            );
      ...
      this.logger.log(
        `-> OPENSEA_SEAPORT receipt parsed: ${OPENSEA_SEAPORT.length}`,
      );
      ...
      if (LR.length) {
        ...
        const redeemLog = receipt.logs.filter(
          (log: any) =>
            log.topics[0].toLowerCase() ===
            '0x63b13f6307f284441e029836b0c22eb91eb62a7ad555670061157930ce884f4e',
        )[0];
        ...
        this.logger.log(':::: No alternate value ::::', { receipt });
      }

      this.logger.log(`-> alternateValue ${alternateValue}`);
      ...
      if (swaps.length) {
        this.logger.log('nftx swap involved in this transaction, ignoring it');
        return;
      }
      ...
      this.logger.log(`-> all done, sending TweetRequest:`, tweetRequest);

      return tweetRequest;
    } catch (err) {
      this.logger.log(`${tokenId} failed to send`, err);
      return null;
    }
  }
}
